...
- `file` — the thing a `particle` identifies: `file = (particle, data)`, nothing more; its `name` and any meta are cyberlinks about it, not parts of it — a document, an image, a model, a neuron's key, an axon's pair of endpoints. the [[fs]] unit; what a neuron reads, makes, links and pays for. data carries no label inside it: its identity is its particle (computed from content), its name is assigned separately.
